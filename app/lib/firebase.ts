...
function validateFirebaseConfig(
  ...
  if (!config.databaseURL) {
    missingFields.push("databaseURL");
  }
  ...
  if (
    ...
    throw new Error(
      "Invalid Firebase Realtime Database URL."
    );
  }
}
...
let persistencePromise: Promise<void> | null =
  null;

/**
 * Explicitly keeps the Firebase Authentication session in
 * browser local storage. This prevents the signed-in admin
 * from being lost during Next.js navigation or a refresh.
 */
